...
This sample provides an environment to display and capture metrics from the educredph in real time. It consists of a docker-compose file that starts a Prometheus and Grafana server setup configured to collect and display metrics for the test network.
...
You will need to have installed **docker-compose with version 1.29 or above** (note that this is higher than the v1.14 requirement requested for the educredph).

## How to use

1. Go to the educredph directory and run bring up the educredph **./network.sh up createChannel**
2. Bring up the Prometheus/Grafana network in the educredph/prometheus-grafana directory and run **docker-compose up -d**
...
5. Deploy a chaincode (i.e. "./network.sh deployCC -ccn basic -ccp ../asset-transfer-basic/chaincode-go -ccl go"), start using the educredph and use the Grafana dashboard to analyse and assess your network performances.
...
- `peer0.org1.educredph.com:9444`
- `peer0.org2.educredph.com:9445`
- `orderer.educredph.com:9443`
